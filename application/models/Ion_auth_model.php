...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->identity_admin_column1 = $this->config->item('identity_admin1', 'ion_auth');
		$this->identity_admin_column2 = $this->config->item('identity_admin2', 'ion_auth');
		$this->identity_teacher_column = $this->config->item('identity_teacher', 'ion_auth');
	...
	public function register_batch($batch_data = array())
	{
		// Memulai event sebelum registrasi
		$this->trigger_events('pre_register');

		// Mengambil konfigurasi manual_activation dari ion_auth
		$manual_activation = $this->config->item('manual_activation', 'ion_auth');
		...
		// Mengambil grup default dari konfigurasi
		$default_group_name = $this->config->item('default_group', 'ion_auth');
		$query = $this->db->get_where($this->tables['groups'], array('name' => $default_group_name), 1)->row();

		// Validasi grup default
		if (!isset($query->id)) {
			$error = $this->set_error("Error: Data can't record, not found group default.");
			return ['error' => $error];
		}

		// Mendapatkan detail grup default
		$default_group = $query;
		...
		foreach ($batch_data as $data) {
			...
			$users_data[] = array(
				$this->identity_column => $data['identity'],
				'username' => $identity,
				'password' => $password,
				'email' => $data['email'],
				...
				'created_on' => time(),
				'active' => ($manual_activation === false ? 1 : 0),
				'salt' => ($this->store_salt ? $salt : null)
			);
		}

		$this->trigger_events('extra_set');

		// Lakukan batch insert ke tabel pengguna
		$inserted_ids = array();
		...
		// Persiapkan data grup jika tidak ada grup yang disediakan
		if (!empty($inserted_ids)) {
			...
			// Lakukan batch insert ke tabel pengguna_grup
			$this->db->insert_batch($this->tables['users_groups'], $groups);
		}

		$this->trigger_events('post_register');

		$inserted_ids_string = implode(',', $inserted_ids);

		return (!empty($inserted_ids_string)) ? $inserted_ids_string : FALSE;
	}
