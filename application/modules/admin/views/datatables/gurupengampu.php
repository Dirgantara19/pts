...
$(document).ready(function() {
    ...
    let table = new DataTable('#table-example3', {
            ...
            'buttons': [{
...

                {
                    ...
                    action: function(e, dt, node, config) {
                        var $a = $("<a>");
                        $a.attr("href",
                            '<?= site_url('admin/export/gurupengampu'); ?>');
                        $("body").append($a);
                        $a[0].click();
                        $a.remove();
                    },
        ...
        ajax: {
            'url': '<?= site_url('admin/datatables/gurupengampu/ajax_list'); ?>',
            'type': 'post'
        },
        deferRender: true,
        columns: [{
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: 'post',
                    dataType: 'json',
                    url: '<?= site_url('admin/datatables/gurupengampu/delete'); ?>',
                    data: {
                        id: data.id
                    },
    ...
    table.on('click', '.update', function(e) {
        ...
        $.post("<?= site_url('admin/datatables/gurupengampu/get_id'); ?>", {
                id: data.id
            }, {
                dataType: 'json'
            })
    ...
    $("#form").on("submit", function(event) {
        ...
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: '<?= site_url('admin/datatables/gurupengampu/save'); ?>',
            data: dataform,
            success: function(data) {
                setTimeout(function() {
                    reload();
    ...
    //     $.ajax({
    //             url: '<?= site_url('admin/datatables/gurupengampu/import'); ?>',
    //             type: 'POST',
    //             data: formData,
    //             dataType: 'json',
    //             contentType: false,
                ...
                if (Array.isArray(response.solution)) {
                    for (var i = 0; i < response.solution.length; i++) {
                        ...
                        if (data.column && data.row) {
                            toastr["error"]("Cells to eliminate: " + data.column + data.row,
                                title_toastr);
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'post',
                        dataType: 'json',
                        url: '<?= site_url('admin/datatables/gurupengampu/bulk_delete'); ?>',
                        data: {
                            array_id: selected
                        },
                        success: function(data) {
