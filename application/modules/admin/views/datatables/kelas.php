<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        let table = new DataTable('#table-example3', {
            ...
            buttons: {
                'buttons': [{
                    ...
                    },
                    {
                        ...
                        action: function(e, dt, node, config) {
                            var $a = $("<a>");
                            $a.attr("href", '<?= base_url('admin/export/kelas'); ?>');
                            $("body").append($a);
                            $a[0].click();
                            $a.remove();
                        },
            ...
            'ajax': {
                'url': "<?= base_url('admin/datatables/kelas/ajax_list'); ?>",
                'type': 'post'
            },
            'deferRender': true,
            'columnDefs': [{
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'post',
                        dataType: 'json',
                        url: '<?= base_url('admin/datatables/kelas/delete'); ?>',
                        data: {
                            id: data.id
                        },
        ...
        table.on('click', '.update', function(e) {
            ...
            $.ajax({
                url: "<?= base_url('admin/datatables/kelas/get_id'); ?>",
                data: {
                    id: data.id
                },
                type: 'post',
        ...
        $("#form").on("submit", function(event) {
            ...
            $.ajax({
                type: 'post',
                dataType: 'json',
                url: '<?= base_url('admin/datatables/kelas/save'); ?>',
                data: dataform,
                success: function(data) {
                    modal.modal('hide');
                    if (data.success) {
        ...
        $("#form-import").on("submit", function(event) {
            ...
            $.ajax({
                type: 'post',
                dataType: 'json',
                url: '<?= base_url('admin/datatables/kelas/import'); ?>',
                data: formData,
                contentType: false,
                processData: false,
                success: function(response) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'post',
                        dataType: 'json',
                        url: '<?= base_url('admin/datatables/kelas/bulk_delete'); ?>',
                        data: {
                            array_id: selected
                        },
                        success: function(response) {
